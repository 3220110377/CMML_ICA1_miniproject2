...
# Construct a normal distribution based on expN and expVar, from which a random 
# sample of exploration time is taken and resampled if it is negative.
...
simulation <- function(Param.df, type, exp_type, save=F, savepath="",
  ...
  # Initialise familiarity based on payoff
  ...
  for (Subject in 1:subjectnum) {
    ...
    # RT is the reaction time, which is the amount of time it takes to make a selection, 
    # e.g. rt1 is the time it takes to select the first item.
    # clock is the total time of the experiment 
    # ctime is used to calculate each reaction time
    
    # AS is attention shift, which is the number of times attention is shifted to this item.
    # In the experiment, the subject will look at multiple pictures, you look at picture A, 
    # then look at picture B, then look at picture A, and your attention will be on picture a twice
    ...
    # schemainfo contains Confidence and Exploration time
    # conN (mean of confidence), 
    ...
    #### the presented item in exploration phase are generated ####
    ...
    #### Experimental parameter initialisation ####
    
    ThisRound = 1
    clock = 0 # record time, in second
    break_happens = 0 # flag to indicate break happens
    explorationtime = 600 # the exploration time (s), 10 min for first exploration
    ...
    # Set the parameter to determine that clock will be initialised only once.
    not.updated <- T
    
    #### Experimental parameter initialisation ####
    
    # Total simulation time 4500s
    while (clock < 4500){
      print(paste("Subject:", Subject,"Round:", ThisRound,"Time:", clock))
      print(paste("break_happens:",break_happens,"risk:",risk,"type:", type))
      
      # If only after is to be simulated and has not been updated, initialise clock
      ...
      # BREAK LOGIC
      # The before phase is 1500s, if no break has occurred yet.
      if (break_happens == 0 & clock >= 1500){
        
        # If only the after phase is to be modelled, 
        # the confidence and general confidence obtained in the before phase are read.
        if(sim.mode=="after"){
          ...
          # Parameters using only the last time
        ...
        explorationtime = 300 # first exploration time is 5 min right after break
      ...
      # sample the confidence prior the exploration
      ...
      explorationtime = 180 ## usually, the exploration time is 3 min
      ...
      # Select the schema shown in the first round
      ...
      while (decisiontime <= 2){ ## i.e. 2 phase to choose item
        ...
        #### sample the schema-based and generic confidence ####
        ...
        while (Finish <= 4){ ## i.e. we start to choose 4 item
          ...
          # If there is no current attention object, start random attention based on p
          if (attention == 0) {
            
            # timevar means revisit
            Item_EI$timevar = 1-(1/(exp(timevar*Item_EI$recovery)))
            
            # If there is only one item left attention is directed to that item.
            if (sum(Item_EI$decision == 0) == 1){ 
              attention = Item_EI$ID[Item_EI$decision == 0]
            }else{
            
            # The probability of noticing p is calculated based on evidence, DM and timevar 
            # (where evidence is the cumulative confidence level of the observation, DM is the payoff, and timevar is previously noticed)
            ...
            # OB is the number of items looked at, AS is how many times the item was looked at
          ...
          while (shift == 0) { ### i.e., when the attention does not shift
            ...
            # For all items that are not currently attended to and are not selected, evidence is reduced by multiplying decay_speed by
            #### evidence decay ####
            ...
            if (Item_EI$evidence[Item_EI$decision == 0 & 
              ...
              if (any(Item_EI$evidence[Item_EI$decision == 0 & 
                                     Item_EI$ID == attention & 
                                     Item_EI$status == 1] >= 
              ...
              # If evidence exceeds the final threshold and has not already been selected, it is selected
            ...
            # (If the object being attended to does not reach the threshold, start extracting whether to divert attention)
            
            # To divert attention, update the parameters
            if (shift == 1){
        ...
        # Using the ratio of payoff to max payoff instead of AC to update confidence
        # -2 is to sharpen the ratios and cause more impact
        schemainfo$conN[schemainfo$schemaID %in% unique(Schema_res)] = 
          schemainfo$conN[schemainfo$schemaID %in% unique(Schema_res)] + 
          Beta_N*((Outputs_cho$payoff[ThisPhase]-2)/(max(Decitem$payoff)-2) - Schema_EI$Scon[Schema_EI$schemaID %in% unique(Schema_res)])
        
        schemainfo$conVar[schemainfo$schemaID %in% unique(Schema_res)] = 
          schemainfo$conVar[schemainfo$schemaID %in% unique(Schema_res)] + 
          Beta_Var*(abs(Schema_EI$Scon[Schema_EI$schemaID %in% unique(Schema_res)] - (Outputs_cho$payoff[ThisPhase]-2)/(max(Decitem$payoff)-2)) - 
                      schemainfo$conVar[schemainfo$schemaID %in% unique(Schema_res)])
        
        Gcon$conN = Gcon$conN + Beta_gN*((Outputs_cho$payoff[ThisPhase]-2)/(max(Decitem$payoff)-2) - unique(Schema_EI$Gcon))
        Gcon$conVar = Gcon$conVar + Beta_gVar*(abs((Outputs_cho$payoff[ThisPhase]-2)/(max(Decitem$payoff)-2) - unique(Schema_EI$Gcon)) - Gcon$conVar)
...
Param.df <- data.frame(Subject = 1:subjectnum,
                       ...
                       Phi = rep(30,subjectnum),
                       # Phi = rep(c(10, 20, 30, 40, 50), each = 100),
                       # The extent to which the three parameters evidence, DM, and timevar affect p
                       decay_speed = rep(0.999,subjectnum),
                       decay_speed_thres = rep(0.999,subjectnum), 
                       thres_item_inter  = rep(6,subjectnum),
                       thres_item_final = rep(13.75,subjectnum), # decision threshold
                       theta_shift = rep(3,subjectnum), # Probability of a random shift in attention 1/n+1
                       # theta_shift = rep(c(1, 2, 3, 4, 5, 6), each = 100),
...
res <- simulation(Param.df, "L", "data/painting", save = T,
                  savepath = "res/before", sim.mode="before",
                  before.path = file.path("res/before", "L"),
                  scale.confi.init = T)

res <- simulation(Param.df, "L", "data/painting", save = T,
                  savepath = "res/after", sim.mode="after",
                  before.path = file.path("res/before", "L"),
                  scale.confi.init = T)
